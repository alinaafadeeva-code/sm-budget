...
def save_expenses(records: list, replace: bool = True):
    """Сохраняет расходы.
    replace=True  — заменяет старые записи за то же юрлицо + год + месяц (по умолчанию).
    replace=False — добавляет к существующим данным (старые не удаляются).
    """
